...
class Hands:
    ...
    def save_masks(self, save_dir, im, img_id, mess=''):
        ...
        cv2.imwrite(save_dir+'/2_'+str(self.id)+'_' +
                    img_id.split('.')[0]+'.png', ims)

        if self.has_first:
            ims[:, :, :] = 0
            ims[self.obj_masks, :] = 255
            cv2.imwrite(save_dir+'/3_'+str(self.id)+'_' +
                        img_id.split('.')[0]+'.png', ims)

            if self.has_second:
                ims[:, :, :] = 0
                ims[self.second_obj_masks, :] = 255
                cv2.imwrite(save_dir+'/5_'+str(self.id)+'_' +
                            img_id.split('.')[0]+'.png', ims)
    ...
    def get_json(self):
        ...
        info['obj_bbox'] = [
            str(x) for x in self.obj_bbox] if self.obj_bbox is not None else None
        info['obj_touch'] = str(self.obj_touch)
        info['obj_touch_scores'] = parse_touch(
            self.obj_touch_score) if self.has_first else None
        info['second_obj_bbox'] = [str(
            x) for x in self.second_obj_bbox] if self.second_obj_bbox is not None else None
    ...
    def get_second_bbox_tuple(self):
...
# read in the image, get the prediction, and format the output
def deal_output(im, predictor):
    outputs = predictor(im)

    pred_boxes = outputs["instances"].get(
        "pred_boxes").tensor.to("cpu").detach().numpy()
    pred_dz = outputs["instances"].get("pred_dz").to("cpu").detach().numpy()
    pred_classes = outputs["instances"].get(
        "pred_classes").to("cpu").detach().numpy()
    pred_scores = outputs["instances"].get("scores").to("cpu").detach().numpy()
    pred_masks = outputs["instances"].get(
        "pred_masks").to("cpu").detach().numpy()
    ...
    for i in range(len(pred_classes)):
        if pred_classes[i] == 0:
            curr_hand = Hands(hand_id=count, hand_bbox=pred_boxes[i], hand_mask=pred_masks[i], contactState=int(contact_state[i].item(
            )), hand_side=hand_side[i].item(), grasp=grasp[i].item(), pred_score=pred_scores[i], grasp_scores=grasp_scores[i])
            count = count+1

            if interaction[i] >= 0:
                obj_id = int(interaction[i])

                curr_hand.set_first_obj(obj_bbox=pred_boxes[obj_id], obj_touch=touch_type[obj_id].item(
                ), obj_masks=pred_masks[obj_id], pred_score=pred_scores[obj_id], touch_scores=touch_scores[obj_id])

                if interaction[obj_id] >= 0:
                    second_obj_id = int(interaction[obj_id])

                    curr_hand.set_second_obj(
                        obj_bbox=pred_boxes[second_obj_id], obj_masks=pred_masks[second_obj_id], pred_score=pred_scores[second_obj_id])
...
def main():
    ...
    # The directory where the results will be saved at
    parser.add_argument("--save_dir", default=f"/launch/evacheng/vis")
    # If True, the visualization of each image will be saved at save_dir
    ...
    for test_img in tqdm(images):
        ...
        if save_img:
            im = vis_per_image(
                im, img['predictions'], im_name+'.png', save_mask_dir, use_simple=False)
            save_path = os.path.join(save_dir, im_name+'.png')
            im.save(save_path)

        res["images"].append(img)

    # a json file which stores all prediction results will be stored at save_dir
    f = open(json_path, 'w')
