...
_datasets_root = "/y/evacheng/val_sample"
register_coco_instances(name = '100DOH_hand_VAL', metadata = {}, json_file = os.path.join(_datasets_root, "annotations", "val.json"), image_root = os.path.join(_datasets_root, "val"))
    ...
    def _derive_coco_results(self, coco_eval, iou_type, class_names=None):
        ...
        for idx, name in enumerate(class_names):
            ...
            for i in range(len(precision)):
                curr_pre = precisions[i, :, idx, 0, -1]
                curr_pre = curr_pre[curr_pre > -1]
                curr_ap = np.mean(curr_pre) if curr_pre.size else float("nan")
                full_ap["AP"+str(50+5*i)] = curr_ap
            
            full_results_per_categtory[name] = full_ap
        ...
        for idx, name in enumerate(class_names):
            ...
            full_results_per_categtory[name]["mAP"] = ap
        ...
        f = open(os.path.join(self.output_dir))
        return results
...
def main(args):
    ...
    cfg.OUTPUT_DIR = "/home/evacheng/val_results/355k_train_Ayda_val_sam/blur_on_blur/"
    cfg.MODEL.WEIGHTS = "/y/evacheng/final_weights/final_on_blur_model_0354999.pth"
    cfg.freeze()
